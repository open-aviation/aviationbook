...
\overrightarrow{OP} = -(R_T +h) \overrightarrow{k}_h$${#eq-position_NED_sphere}
...
\frac{d\overrightarrow{OP}}{dt}_{/Ox_my_mz_e} = -(R_T+h)\frac{d\mu}{dt}\frac{d\overrightarrow{k}_h}{d\mu} - \dot{h} \overrightarrow{k}_h = \dot{\mu}(R_T+h) \overrightarrow{\imath}_h - \dot{h} \overrightarrow{k}_h$$ {#eq-deriv_position_NED_sphere}
...
- \dot{h}
\end{pmatrix}$$ {#eq-coor_deriv_vect_op}
...
e^2=1-\frac{b^2}{a^2}$$ {#eq-eccentricity}
...
$$\label{eq:ELLIPSE}
\frac{x^2}{a^2} + \frac{z^2}{b^2} = 1$$ {#eq-ellipse}
...
\frac{dz}{dx} = - \frac{b^2}{a^2}\frac{x}{z}$$ {#eq-tangent_to_ellipse}

The direction of the normal is then given by @eq-normal_direction:

$$\label{eq:NORMAL_DIRECTION}
\tan \mu = \frac{a^2}{b^2}\frac{z}{x}$$ {#eq-normal_direction}

From the definition of geocentric latitude, we can also see that:

$$\label{eq:CHI_GEOCENTRIC}
\tan \chi = \frac{z}{x}$$ {#eq-chi_geocentric}
...
$$\label{eq:CHI_MU}
\boxed{\tan \chi = \frac{b^2}{a^2} \tan \mu}$$ {#eq-chi_mu}
...
x= a \cos u \\
z= b \sin u
\end{array}$$ {#eq-coor_point_on_ellipse}
...
$$\label{eq:COOR_IN_PLANE_MU}
\begin{array}{l}\{\overrightarrow{ON}\}_{Oxz}\\ \\ \\\end{array} \Bigg|
\begin{array}{l}
x= \mathcal{N} \cos \mu\\
z= \mathcal{N} (1- e^2)\sin \mu
\end{array}$$ {#eq-coor_in_plane_mu}
...
%)^{\frac{1}{2}}}$$ {#eq-ellipsoid_normal_distance}
...
\end{array}$$ {#eq-coor_ellipsoid_meridian_N}
...
z_e= \mathcal{N} (1-e^2)\sin \mu
\end{array}$$ {#eq-coor_ellipsoid_ECEF_N}
...
\end{array}$$ {#eq-coor_ellipsoid_meridian_P}
...
z_e= \left[\mathcal{N} (1-e^2)+h\right]\sin \mu
\end{array}$$ {#eq-coor_ellipsoid_ECEF_P}
...
$$\label{eq:DERIV_NADIR}
\frac{d\overrightarrow{ON}}{dt}_{/Ox_my_mz_e} = \dot{\mu}R_M \overrightarrow{\imath}_h$$ {#eq-deriv_nadir}

where $R_M$ is the meridian radius of curvature (see @fig-curvature_radii):

$$\label{eq:MERIDIAN_CURVATURE_RADIUS}
\boxed{R_M= \frac{a(1-e^2)}{\left(1-e^2\sin ^2 \mu\right)^{\frac{3}{2}}}}$$ {#eq-meridian_curvature_radius}
