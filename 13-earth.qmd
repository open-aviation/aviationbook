...
the orthonormal vectors of the ECEF reference frame $Ox_ey_ez_e$.
  ...
  ------------------------------------------------------------------------------------------------------------------------------------------------
  ![[\[fig:SPHERIC\_EARTH\]]{#fig:SPHERIC_EARTH label="fig:SPHERIC_EARTH"} Spherical Earth model.](images/earth_model/Terre_spherique.jpg){width="8cm"}
  ------------------------------------------------------------------------------------------------------------------------------------------------

In this coordinate system, the position of a point $P$ is given by its
latitude, longitude, and distance from the center of Earth, or more
simply its altitude $h$ above the surface of the globe, as illustrated
in
Figure [\[fig:SPHERIC\_EARTH\]](#fig:SPHERIC_EARTH){reference-type="ref"
reference="fig:SPHERIC_EARTH"}. The latitude, denoted $\mu$ in the
following, is the angle between the equatorial plane $x_eOy_e$ and
$\overrightarrow{OP}$. The longitude, denoted $\lambda$, is the angle
between the Greenwich meridian plane $x_eOz_e$ and the meridian plane
...
represented on Figure [\[fig:NED\]](#fig:NED){reference-type="ref"
reference="fig:NED"}.
...
system -- of a point $P$ at altitude $h$ are given by
equation  [\[eq:COOR\_SPHERE\]](#eq:COOR_SPHERE){reference-type="eqref"
reference="eq:COOR_SPHERE"}:

$$\label{eq:COOR_SPHERE}
\begin{array}{l}\{\overrightarrow{OP}\}_{\text{\it ECEF}}\\ \\ \\\end{array} \Bigg|
\begin{array}{l}
...
be expressed very simply by
equation [\[eq:POSITION\_NED\_SPHERE\]](#eq:POSITION_NED_SPHERE){reference-type="eqref"
reference="eq:POSITION_NED_SPHERE"}, and the coordinates by
equation [\[eq:COOR\_NED\_SPHERE\]](#eq:COOR_NED_SPHERE){reference-type="eqref"
reference="eq:COOR_NED_SPHERE"}:

$$\label{eq:POSITION_NED_SPHERE}
\overrightarrow{OP} = -(R_T +h) \overrightarrow{k}_h$$

$$\label{eq:COOR_NED_SPHERE}
\begin{array}{l}\{\overrightarrow{OP}\}_{\text{\it NED}}\\ \\ \\\end{array} \Bigg|
\begin{array}{l}
x_h= 0\\
y_h= 0\\
z_h= - (R_T+h)
...
fixed, is given
in [\[eq:SPEED\_SPHERE\]](#eq:SPEED_SPHERE){reference-type="eqref"
reference="eq:SPEED_SPHERE"}, taking into account the angular speed
$\overrightarrow{\Omega}_{Ox_my_mz_e/\text{\it ECEF}} = \dot{\lambda} \overrightarrow{k}_e$
of the $Ox_my_mz_e$ system around the axis $Oz_e$:

$$\label{eq:SPEED_SPHERE}
\begin{aligned}
\frac{d\overrightarrow{OP}}{dt}_{/\text{\it ECEF}} = \frac{d\overrightarrow{OP}}{dt}_{/Ox_my_mz_e} + \overrightarrow{\Omega}_{Ox_my_mz_e/\text{\it ECEF}} \wedge\overrightarrow{OP}\\
\end{aligned}$$

The derivative $\frac{d\overrightarrow{OP}}{dt}_{/Ox_my_mz_e}$ is
obtained simply by
deriving [\[eq:POSITION\_NED\_SPHERE\]](#eq:POSITION_NED_SPHERE){reference-type="eqref"
reference="eq:POSITION_NED_SPHERE"} assuming $Ox_my_mz_e$ is fixed. With
...
$$\label{eq:DERIV_POSITION_NED_SPHERE}
...
By
combining [\[eq:DERIV\_POSITION\_NED\_SPHERE\]](#eq:DERIV_POSITION_NED_SPHERE){reference-type="eqref"
reference="eq:DERIV_POSITION_NED_SPHERE"}
and [\[eq:SPEED\_SPHERE\]](#eq:SPEED_SPHERE){reference-type="eqref"
reference="eq:SPEED_SPHERE"}, the velocity with respect to the ECEF
reference frame is then expressed (see
Eq. [\[eq:DERIV\_VECT\_OP\]](#eq:DERIV_VECT_OP){reference-type="eqref"
reference="eq:DERIV_VECT_OP"}) in a simple form in the basis of vectors
$(\overrightarrow{\imath}_h, \overrightarrow{\jmath}_h, \overrightarrow{k}_h)$
associated with the local horizontal frame NED.

$$\label{eq:DERIV_VECT_OP}
 \begin{aligned}
\frac{d\overrightarrow{OP}}{dt}_{/\text{\it ECEF}} & =  \dot{\mu}(R_T+h) \overrightarrow{\imath}_h - \dot{h} \overrightarrow{k}_h + \left(\dot{\lambda} \overrightarrow{k}_e\right) \wedge\left(-(R_T +h) \overrightarrow{k}_h\right)\\
...
"$/\text{\it ECEF}$" means *"with respect to the Earth's reference
frame"* and $\{.\}_{\text{\it NED}}$ means *"in the NED (North, East,
Down) coordinate system"*, the coordinates of the velocity with respect
to the Earth are given by
Equation [\[eq:DERIV\_VECT\_OP\]](#eq:DERIV_VECT_OP){reference-type="eqref"
reference="eq:DERIV_VECT_OP"}.

$$\label{eq:COOR_DERIV_VECT_OP}
\left\{\frac{d\overrightarrow{OP}}{dt}_{/\text{\it ECEF}}\right\}_{\text{\it NED}}  =
\begin{pmatrix}
\dot{\mu}(R_T+h)\\
...
equation [\[eq:ECCENTRICITY\]](#eq:ECCENTRICITY){reference-type="eqref"
reference="eq:ECCENTRICITY"} : $$\label{eq:ECCENTRICITY}
e^2=1-\frac{b^2}{a^2}$$

The equation of the ellipse is recalled
in [\[eq:ELLIPSE\]](#eq:ELLIPSE){reference-type="eqref"
reference="eq:ELLIPSE"}, where $z$ is the minor axis (the polar axis),
and $x$ is the major axis of the ellipse : $$\label{eq:ELLIPSE}
...
surface of the ellipsoid of revolution (see
Figure [\[fig:ELLIPSOID\_EARTH\]](#fig:ELLIPSOID_EARTH){reference-type="ref"
reference="fig:ELLIPSOID_EARTH"}). Note that this perpendicular line
...
of a point $N'$ (see
Fig. [\[fig:ELLIPSE\_LATITUDES\]](#fig:ELLIPSE_LATITUDES){reference-type="ref"
reference="fig:ELLIPSE_LATITUDES"}) on a circle with center $O$ and
...
latitudes are illustrated in
Fig. [\[fig:ELLIPSE\_LATITUDES\]](#fig:ELLIPSE_LATITUDES){reference-type="ref"
reference="fig:ELLIPSE_LATITUDES"}.
...
Therefore, in
figure [\[fig:ELLIPSE\_LATITUDES\]](#fig:ELLIPSE_LATITUDES){reference-type="ref"
reference="fig:ELLIPSE_LATITUDES"}, we observe that
$\frac{HN}{OH} = \frac{b}{a} \frac{HN'}{OH}$. We immediately deduce the
relation [\[eq:CHI\_U\]](#eq:CHI_U){reference-type="eqref"
reference="eq:CHI_U"} between the geocentric latitude $\chi$ and the
...
Moreover, by
differentiating [\[eq:ELLIPSE\]](#eq:ELLIPSE){reference-type="eqref"
reference="eq:ELLIPSE"}, we obtain the slope of the tangent to the
ellipse at point $N$, expressed
in [\[eq:SLOPE\_TANGENT\_TO\_ELLIPSE\]](#eq:SLOPE_TANGENT_TO_ELLIPSE){reference-type="eqref"
reference="eq:SLOPE_TANGENT_TO_ELLIPSE"}:
$$\label{eq:SLOPE_TANGENT_TO_ELLIPSE}
\frac{dz}{dx} = - \frac{b^2}{a^2}\frac{x}{z}$$

The direction of the normal is then given
by [\[eq:NORMAL\_DIRECTION\]](#eq:NORMAL_DIRECTION){reference-type="eqref"
reference="eq:NORMAL_DIRECTION"}: $$\label{eq:NORMAL_DIRECTION}
\tan \mu = \frac{a^2}{b^2}\frac{z}{x}$$

From the definition of geocentric latitude, we can also see that:
$$\label{eq:CHI_GEOCENTRIC}
\tan \chi = \frac{z}{x}$$

From
Equations [\[eq:NORMAL\_DIRECTION\]](#eq:NORMAL_DIRECTION){reference-type="eqref"
reference="eq:NORMAL_DIRECTION"}
and [\[eq:CHI\_GEOCENTRIC\]](#eq:CHI_GEOCENTRIC){reference-type="eqref"
reference="eq:CHI_GEOCENTRIC"} we deduce the relation between the
geocentric latitude $\chi$ and the geodetic latitude $\mu$ :
$$\label{eq:CHI_MU}
\boxed{\tan \chi = \frac{b^2}{a^2} \tan \mu}$$

By combining [\[eq:CHI\_U\]](#eq:CHI_U){reference-type="eqref"
reference="eq:CHI_U"}
and [\[eq:CHI\_MU\]](#eq:CHI_MU){reference-type="eqref"
reference="eq:CHI_MU"}, we can easily deduce the
relation [\[eq:MU\_U\]](#eq:MU_U){reference-type="eqref"
reference="eq:MU_U"} between the parametric latitude $u$ and the
geodetic latitude $\mu$: $$\label{eq:MU_U}
\tan u = \frac{b}{a} \tan \mu$$
...
The point $N'$ of
Fig. [\[fig:ELLIPSE\_LATITUDES\]](#fig:ELLIPSE_LATITUDES){reference-type="ref"
reference="fig:ELLIPSE_LATITUDES"} is on a circle of radius $a$. Its
...
given by
Equation [\[eq:COOR\_POINT\_ON\_ELLIPSE\]](#eq:COOR_POINT_ON_ELLIPSE){reference-type="eqref"
reference="eq:COOR_POINT_ON_ELLIPSE"}:

$$\label{eq:COOR_POINT_ON_ELLIPSE}
\begin{array}{l}\{\overrightarrow{ON}\}_{Oxz}\\ \\ \\\end{array} \Bigg|
\begin{array}{l}
x= a \cos u \\
z= b \sin u 
...
Furthermore, considering the geodetic latitude $\mu$ and the great
normal $\mathcal{N} = IN$, we observe that $x= \mathcal{N} \cos\mu$.
Taking into account this expression
and [\[eq:MU\_U\]](#eq:MU_U){reference-type="eqref"
reference="eq:MU_U"}, the coordinate $z$ is rewritten as a function of
the geodetic latitude as follows:
...
By introducing the
eccentricity [\[eq:ECCENTRICITY\]](#eq:ECCENTRICITY){reference-type="eqref"
reference="eq:ECCENTRICITY"}, we finally obtain the coordinates of $N$
as a function of the geodetic latitude $\mu$, expressed in the following
equation [\[eq:COOR\_IN\_PLANE\_MU\]](#eq:COOR_IN_PLANE_MU){reference-type="eqref"
reference="eq:COOR_IN_PLANE_MU"}: $$\label{eq:COOR_IN_PLANE_MU}
\begin{array}{l}\{\overrightarrow{ON}\}_{Oxz}\\ \\ \\\end{array} \Bigg|
\begin{array}{l}
x= \mathcal{N} \cos \mu\\
...
#### Expression of the great normal

The expression of the great normal $\mathcal{N}$ is simply found
from [\[eq:COOR\_IN\_PLANE\_MU\]](#eq:COOR_IN_PLANE_MU){reference-type="eqref"
reference="eq:COOR_IN_PLANE_MU"} and the equation of the
ellipse [\[eq:ELLIPSE\]](#eq:ELLIPSE){reference-type="eqref"
reference="eq:ELLIPSE"}: $$\begin{aligned}
& \frac{x^2}{a^2} + \frac{z^2}{b^2} = 1\\
...
$$\label{eq:ELLIPSOID_NORMAL_DISTANCE}
...
different reference points and axis systems are described in
Figure [\[fig:ELLIPSOID\_AXIS\_SYSTEMS\]](#fig:ELLIPSOID_AXIS_SYSTEMS){reference-type="ref"
reference="fig:ELLIPSOID_AXIS_SYSTEMS"}.
...
Figure [\[fig:ELLIPSOID\_AXIS\_SYSTEMS\]](#fig:ELLIPSOID_AXIS_SYSTEMS){reference-type="ref"
reference="fig:ELLIPSOID_AXIS_SYSTEMS"}), where the point $N$ (nadir) is
...
$$\label{eq:COOR_ELLIPSOID_MERIDIAN_N}
...
$$\label{eq:COOR_ELLIPSOID_ECEF_N}
\begin{array}{l}\{\overrightarrow{ON}\}_{\text{\it ECEF}}\\ \\ \\\end{array} \Bigg|
\begin{array}{l}
...
given by
equation [\[eq:ELLIPSOID\_NORMAL\_DISTANCE\]](#eq:ELLIPSOID_NORMAL_DISTANCE){reference-type="eqref"
reference="eq:ELLIPSOID_NORMAL_DISTANCE"}.

#### Coordinates of point $P$ at altitude $h$

$$\label{eq:COOR_ELLIPSOID_MERIDIAN_P}
...
$$\label{eq:COOR_ELLIPSOID_ECEF_P}
\begin{array}{l}\{\overrightarrow{OP}\}_{\text{\it ECEF}}\\ \\ \\\end{array} \Bigg|
\begin{array}{l}
...
where $R_M$ is the meridian radius of curvature (see
Figure [\[fig:CURVATURE\_RADII\]](#fig:CURVATURE_RADII){reference-type="ref"
reference="fig:CURVATURE_RADII"}):
$$\label{eq:MERIDIAN_CURVATURE_RADIUS}
...
passing through the point $N$ (see
Figure [\[fig:CURVATURE\_RADII\]](#fig:CURVATURE_RADII){reference-type="ref"
reference="fig:CURVATURE_RADII"}), we finally obtain the
expression [\[eq:DERIV\_VECT\_OP\_ELLIPSOID\]](#eq:DERIV_VECT_OP_ELLIPSOID){reference-type="eqref"
reference="eq:DERIV_VECT_OP_ELLIPSOID"} :

$$\label{eq:DERIV_VECT_OP_ELLIPSOID}
\boxed{\left\{\frac{d\overrightarrow{OP}}{dt}_{/\text{\it ECEF}}\right\}_{\text{\it NED}}  =
\begin{pmatrix}
\dot{\mu}(R_M+h)\\
...
by [\[eq:MERIDIAN\_CURVATURE\_RADIUS\]](#eq:MERIDIAN_CURVATURE_RADIUS){reference-type="eqref"
reference="eq:MERIDIAN_CURVATURE_RADIUS"}, and the radius $R_P$ of the
parallel circle is given by the following
equation [\[eq:PARALLEL\_RADIUS\]](#eq:PARALLEL_RADIUS){reference-type="eqref"
reference="eq:PARALLEL_RADIUS"}: $$\label{eq:PARALLEL_RADIUS}
