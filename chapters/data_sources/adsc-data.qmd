...
function adscDirectionError(error, direction = "uplink") {
  const directionLabel = direction === "downlink" ? "downlink" : "uplink";
  return datalink.errorBlock(error, { application: "ADS-C", direction: directionLabel });
}
...
//| echo: false
//| code-copy: false
viewof adscEnvelope = datalink.envelopeInput({
  value: selectedAdscSample,
  ariaLabel: "ADS-C uplink ARINC 622 envelope"
})
```
...
//| echo: false
//| code-copy: false
viewof adscReportEnvelope = datalink.envelopeInput({
  value: selectedAdscReportSample.raw,
  ariaLabel: "ADS-C downlink ARINC 622 envelope"
})
```
...
Finally, ADS-C may also be observed on HF and VDL2 channels, or through aggregators such as [airframes.io](https://airframes.io) and [thebaldgeek's pages](https://tbgmap.airframes.io/). These sources contain more reports over continental areas, including near airports. Because those areas are generally well covered by ADS-B, ADS-C is often a lower priority there.

The most comprehensive tutorials for receiving satellite and non-ADS-B data can be found here: [thebaldgeek.github.io](https://thebaldgeek.github.io)
