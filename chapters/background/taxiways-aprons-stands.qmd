...
Taxiways must also be designed with expandability in mind. Systems should allow for later extensions without major reconstruction, supporting phased airport growth. For instance, from an initial layout for 20000–30000 annual movements to one accommodating 150000–250000 movements.  
...
* **Holding Positions / Holding Points:**  
  ...
  The required distance from the runway holding position to the runway centerline depends on the aerodrome code number and runway type. On runways with CAT II/III approaches, requirements are the most stringent, as \acrfull{OFZ} and \acrfull{ILS} critical/sensitive areas must be respected.  
...
<div class="radius-output">
  ...
  <strong><span id="radiusMeters">126.8</span> m</strong>
</div>
...
<svg id="taxiwaySvg" viewBox="0 0 500 320">
  ...
  <text id="radiusLabel" font-size="18" fill="#333">r_min = 126.8 m</text>
</svg>
:::
...
![Static illustration of the minimum taxiway curve radius for $v_{\text{max}} = 25\,\text{kt}$, resulting in $r_{\text{min}} \approx 126.8\,\text{m}$.](../../images/illustrations/taxiway-radius-static.png){#fig-taxiway-radius-static width=75%}
:::



The **width** of a taxiway, together with its shoulders and strips, is specified in *CS ADR-DSN.D* (see @tbl-twy-dims). The required dimensions depend on the \acrfull{OMGWS} of the critical aircraft. For example, for a Code C aircraft (such as the A320, B737, or A220), the taxiway must have a width of 15 m, the shoulders a width of 25 m, and the strip a width of 52 m. Note that all dimensions refer to edge-to-edge measurements.  

| Code Letter | \acr{OMGWS} | Taxiway | Shoulder       | Strip |
|-------------|-------------|---------|----------------|-------|
...
* **Centerline markings** provide pilots with continuous guidance between the runway and the stand, and vice versa. They are generally painted in yellow and consist of a continuous line approximately 20 centimeters wide. At some airports, centerlines are color-coded on apron areas (e.g., apron taxiways) to improve clarity. For example, at Frankfurt Airport, color coding allows certain taxiways to be used either by one Code E/F aircraft or simultaneously by two Code C aircraft, thereby reducing potential capacity bottlenecks on apron taxiways.
...
* **Contact stands** are directly connected to a passenger terminal building, allowing passengers to board and disembark the aircraft via a jet bridge. Since passengers can move directly between the terminal and the aircraft, contact stands are generally preferred by passengers and perceived as offering higher service quality.  
  ...
  - *Azimuth Guidance for Nose-In Stands (\acr{AGNIS})* is a hybrid between a docking guidance system and a marshaller. \acr{AGNIS} consists of two components: the azimuth guidance unit, which indicates whether the aircraft is aligned with the centerline, and the stop information system, which shows how far the aircraft must taxi to reach the stop point. Both systems function mechanically, using parallax and shutters that make certain display elements visible only from specific angles.  
