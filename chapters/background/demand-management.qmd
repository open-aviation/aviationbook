 ...
 * **Operational demand management:** Temporary reductions in airport or airspace capacity are often managed through \acr{ATFCM} measures. In Europe, this is typically achieved by introducing an \acr{ATFM} regulation, whereby the \acr{EUROCONTROL} Network Manager assigns \acr{CTOT} to affected flights. Delaying aircraft at their departure airport rather than allowing them to queue in the air reduces fuel consumption, emissions, and controller workload while maintaining a safe and orderly traffic flow.
