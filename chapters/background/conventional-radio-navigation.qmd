...
## Navigation angles and reference directions

Before describing the radio aids themselves, it is useful to clarify a few angular navigation terms that are often used together:

- the **heading** angle is where the nose of the aircraft points:\
  the direction of the aircraft longitudinal axis, usually expressed in degrees from north.
- the **track** angle is where the aircraft actually goes over the ground:\
  the projection of its path on the Earth or on a map.
- the **bearing** angle is the direction between two points.\
  It may be a bearing *to* a point, a bearing *from* a point, or a relative bearing measured from the aircraft heading.
- the **course** angle is the desired direction of flight for a route segment,\
  often the intended bearing to the next waypoint.
- the **radial** angle is a station-centred concept:\
  the magnetic bearing *from* a radio navigation aid, especially a [VOR](#vhf-omnidirectional-range-vor) or TACAN, to the aircraft.

Heading, track, bearing, course, and radial must all be interpreted with respect to a north reference. Depending on context, this may be **true north** or **magnetic north**. In aviation, magnetic references remain common because the magnetic compass is a fundamental fallback instrument and because many conventional radio navigation procedures are defined with magnetic bearings.

![source: https://aviation.stackexchange.com/a/96035](../../images/ZUw30.png)

Wind explains why heading and track are not necessarily the same. If there is no wind, an aircraft heading west also tracks west over the ground. With a crosswind, the pilot must point the nose slightly into the wind to maintain the desired ground track. The heading then differs from the track by the drift correction angle.

Radials can be especially confusing because they describe the aircraft position relative to a navigation aid, not necessarily the direction in which the aircraft is flying. An aircraft may be established on a given radial while flying either outbound from the station or inbound toward it. For example, an aircraft flying toward a [VOR](#vhf-omnidirectional-range-vor) on the 140 radial would have an approximate magnetic course of 320 degrees when tracking inbound without wind.
...
Coverage depends strongly on transmitter power and the conductivity of the ground or sea path. Locator \acr{NDB} (L) near airports typically have a range of 15–25 nautical miles, while larger en-route \acr{NDB} can reach 50 nautical miles or more.

For decades \acr{NDB} were widely used for en-route navigation, non-precision approaches, and as locator beacons associated with the [\acrfull{ILS}](#instrument-landing-system-ils). In most operational environments, however, \acr{NDB} are regarded as a legacy aid in the process of being phased out.


## VHF Omnidirectional Range (VOR)

The \acrfull{VOR} is a ground-based azimuth navigation aid that gives an aircraft its magnetic bearing from a station. It became one of the foundations of conventional airways, terminal procedures, and non-precision approaches after the Second World War.

\acr{VOR} operates between **108.0 and 117.95 MHz**, with part of the lower band shared with [\acr{ILS}](#instrument-landing-system-ils) localizers. Each station transmits a reference signal and a variable signal; the phase difference between them encodes the radial on which the aircraft lies. The station also transmits a short Morse-code identifier, normally repeated at regular intervals.

Because \acr{VOR} is a \acr{VHF} line-of-sight system, coverage depends on altitude and terrain. At cruising altitude a station may be usable out to around 200 nautical miles, but local siting matters. Reflections from terrain, buildings, or wind turbines can introduce multipath errors, and directly overhead the station there is a cone of silence where no reliable bearing is available.

Two main designs are used. A \acrfull{CVOR} produces the variable signal with a rotating directional pattern, while a \acrfull{DVOR} synthesises the rotation electronically with a circular antenna array. \acr{DVOR} is less sensitive to site-dependent errors and generally gives more stable bearing information.

Operationally, a single \acr{VOR} radial provides lateral guidance. Combined with [\acrfull{DME}](#distance-measuring-equipment-dme), a VOR/DME site gives both azimuth and slant-range distance; two radials from different stations can also be used to fix position. This made \acr{VOR} central to airways, \acr{SID}, \acr{STAR}, and many non-precision approaches.

The standard total system accuracy is about **±4°**, adequate for conventional navigation but not for modern \acr{RNP} specifications. As aviation moves toward \acr{GNSS}-based \acr{PBN}, many states are reducing their VOR networks while preserving a \acrfull{MON} as a backup in case of satellite-navigation outages.

::: {.callout-tip}
### TODO

- add a link to a page explaining flight inspecitons
...
\acrfull{DME} is a ground-based aid that gives the aircraft its distance from a station. It operates in the \acrfull{UHF} band between **960 and 1215 MHz** and is usually paired with a \acr{VOR} or an [\acr{ILS}](#instrument-landing-system-ils). In a paired installation, selecting the \acr{VOR} or localizer frequency also selects the associated \acr{DME} channel.
...
\acr{DME} accuracy is high for conventional navigation: \acr{ICAO} specifies about **±0.2 NM plus 0.25% of distance**. Coverage is broadly line-of-sight, as for other high-frequency radio aids, and the system is relatively robust against many forms of interference.
...
- add some drawing for geometry?
:::

## Instrument Landing System (ILS)

The \acrfull{ILS} is the most widely used precision approach aid in aviation. It provides both lateral and vertical guidance to enable safe landings in conditions of reduced visibility. An \acr{ILS} consists of three main components:

- the **\acrfull{LOC}** for lateral guidance,
- the **\acrfull{GS}** for vertical guidance, and, historically,
