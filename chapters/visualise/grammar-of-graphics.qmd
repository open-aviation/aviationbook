...
ggplot(country_count, aes(countries, continent, fill = continent)) +
  geom_col() +
  scale_fill_manual(values = palette, guide = "none") +
...
dispersion <- g2007 |>
  group_by(continent) |>
  summarise(mean = mean(gdp_per_capita), stdev = sd(gdp_per_capita),
            .groups = "drop") |>
  mutate(continent = reorder(continent, mean))

ggplot(dispersion, aes(mean, continent, fill = stdev)) +
  geom_col() +
  scale_fill_distiller(palette = "Blues", direction = 1, limits = c(0, 20000)) +
  scale_x_continuous(labels = label_number(scale_cut = cut_short_scale())) +
  labs(x = "mean GDP per capita", y = NULL,
...
```r
ggplot(g2007, aes(gdp_per_capita, continent)) +
  geom_boxplot() +
  scale_x_continuous(labels = label_number(scale_cut = cut_short_scale())) +
...
ggplot(large_europe, aes(year, population, fill = country)) +
  geom_area() +
...
p_top_pop <- ggplot(top_pop, aes(population, country, fill = continent)) +
  geom_col() + scale_fill_manual(values = palette, guide = "none") +
  scale_x_continuous(labels = label_number(scale_cut = cut_short_scale())) +
...
those qualitative choices consistently. **The library supplies the mechanisms**;
...
- Claus O. Wilke, *Fundamentals of Data Visualization*,
  <https://clauswilke.com/dataviz/>, 2019.
:::
