...
Before committing large images or photos, resize and optimize them with
[ImageMagick](https://imagemagick.org/). The `>` suffix prevents smaller images
from being enlarged, `-strip` removes metadata, and `-quality 80` keeps a good
balance between visual quality and file size:

```sh
magick input.jpeg -resize "2000x2000>" -strip -quality 80 output.webp
```
