...
<!--Airspace Classes-->
To ensure that the airspace is managed efficiently and effectively by ATC, flights operating are segmented according to their **flight rules** they are following. Basically, flights are thereby either following so-called *visual flight rules* (VFR) or *instrument flight rules* (IFR). As the name implies, VFR flights follow the principle of "see and avoid". That is, by looking out of the window, VFR flight crews are responsible for ensuring that they are always sufficiently spatially separated from other flights and obstacles. VFR flights can only be conducted when meteorological conditions permit. Among other things, it must be ensured that a certain minimum visibility is guaranteed in an airspace. Furthermore, pilots of VFR flights are prohibited from flying closer than a certain distance to clouds, which implies requirements regarding clouds. In contrast, flights operating under IFR must meet less stringent meteorological requirements. Crews of IFR flights can no longer ensure see and avoid of other flights and obstacles, because for example the visibility conditions do not allow this. On the contrary, crews of IFR flights do not look out of the window, but exclusively at their flight instruments. ATC is responsible for separation, both from other flights and from obstacles.

As annother means to ensure safe and expeditious flow of air traffic, ATC often segments its airspace into a number of different, smaller airspace blocks. These airspace blocks are further assigned a specific **airspace class**, which clearly defines which types of flight may use the airspace, under which circumstances they may enter this airspace and which services are offered to aircraft and/or aircraft crews. In [ICAO Annex 11][Annex_11] werden in insgesamt 7 Luftraumklassen, Class A to Class G, definiert. Diese Luftraumklassen können in einer ersten groben Unterteilung in controlled vs. uncontrolled airspace unterteilt werden, wobei der Begriff controlled darauf hinweist, dass in einem Luftraum von der ATC Services angeboten werden.
